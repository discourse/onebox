...
describe Onebox::Engine::TwitterStatusOnebox do
  ...
  context "with twitter client" do
    before(:each) do
      ...
      allow(@twitter_client).to receive("status").and_return mockTwitterAPIReponse
  ...
  context "with quoted tweet" do
    shared_examples_for "#to_html" do
      ...
      it "gets the correct timestamp" do
        expect(onebox.send(:timestamp)).to eq("10:45 PM - 13 May 2019")
      end

      it "includes name" do
        expect(html).to include("Metallica")
      end
      ...
      it "includes user avatar" do
        expect(html).to include("profile_images/766360293953802240/kt0hiSmv_400x400.jpg")
      end
    end
    ...
    context 'with twitter client' do
      ...
      let(:quoted_tweet_api_response) do
        {
      ...
      #tweetty[:quoted_status][:full_text]
      before(:each) do
        @twitter_client = double("TwitterClient")
        allow(@twitter_client).to receive("status").and_return quoted_tweet_api_response

        allow(@twitter_client).to receive(:prettify_tweet).and_return tweet_content
        allow(@twitter_client).to receive(:twitter_credentials_missing?).and_return false
        allow(@twitter_client).to receive(:prettify_number).and_return "1.7K"
